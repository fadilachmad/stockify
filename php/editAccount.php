...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit'])) {
  ...
  if (!empty($username_input) && !empty($email) && filter_var($email, FILTER_VALIDATE_EMAIL)) {
    $query = "UPDATE users SET user_bio = ?";
    $stmt = $pdo->prepare($query);

    if ($stmt) {
      $stmt->bindValue(1, $user_bio, PDO::PARAM_STR);
      $stmt->execute();
    ...
    <main class="m-3 col-span-10">
      ...
      <div
        ...
        <div class="profile bg-compliment rounded-sm size-64 flex">
          <ion-icon
            name="person-circle"
            class="size-full text-primary"></ion-icon>
        </div>
          ...
          <form action="" method="POST">
            ...
            <div class="bio mb-4">
              ...
              <textarea
                ...
                placeholder="Write something about yourself..."><?= htmlspecialchars($user['user_bio']); ?></textarea>
            </div>
